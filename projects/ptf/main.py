...
def readCommand(argv):
    ...
    parser.add_option('-e', '--epi_step', dest='epi_step', type='int',
                      help=default('the steps of each episode'), default=500)
    parser.add_option('-g', '--game', dest='game',
                      help=default('use which GAME to play'), default='pinball')
    parser.add_option('-a', '--alg', dest='algorithm',
                      help=default('use which algorithm to play'), default='dqn')
    parser.add_option('-c', '--alg_conf', dest='algorithm_config',
                      help=default('algorithm config'), default='dqn_conf')
    parser.add_option('-d', '--env_conf', dest='environment_config',
                      help=default('Environment config'), default='pinball_conf')
    parser.add_option('-s', '--seed', dest='seed', type='int',
                      help=default('the seed of tf'), default=1)
    ...
    args['results_path'] = options.path  #"results/" + args['algorithm'] + "_" + args['game'] + "/" + t + "/"
...
def get_space(env):
    if type(env.action_space) is gym.spaces.discrete.Discrete:
        ...
        raise Exception('action space is not a valid type')
